...
from GTs_baselines.nodeformer import NodeFormer
from GTs_baselines.network.multi_model import MultiModel
from GTs_baselines.nagphormer import TransformerModel
from GTs_baselines.goat import Transformer
...
def parse_method(args, n, c, d, device):
    if args.model == "MPNN":
        model = MPNNs(d, args.hidden_channels, c, local_layers=args.local_layers, dropout=args.dropout, 
            heads=args.num_heads, pre_ln=args.pre_ln, pre_linear=args.pre_linear, res=args.res, ln=args.ln, bn=args.bn, jk=args.jk, gnn=args.gnn, 
            defence_sigma=args.defence_sigma, defence_k=args.defence_k, defence_p=args.defence_p, defence_limit=args.defence_limit).to(device)
        ...
        gnn = GCN(in_channels=d,
            ...
            dropout=args.dropout)
        model = SGFormer(d, args.hidden_channels, c, num_layers=args.ours_layers, alpha=args.alpha, dropout=args.ours_dropout, 
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--global_dropout', type=float, default=0.0)
    # sgformer
    ...
    parser.add_argument('--ours_dropout', type=float, default=0.2)
    parser.add_argument('--ours_weight_decay', type=float, default=0.001)
    # nagphormer
    ...
    # defence
    parser.add_argument('--defence_auc', type=float, default=1.0)
    parser.add_argument('--defence_auc_initial', type=float, default=1.0)
    parser.add_argument('--defence_relu', action='store_true')
    parser.add_argument('--defence_l1', action='store_true')
    parser.add_argument('--defence_l2', action='store_true')
    parser.add_argument('--defence_positive', action='store_true')
    parser.add_argument('--defence_all', action='store_true')

    parser.add_argument('--defence_sigma', type=float, default=0.0)
    parser.add_argument('--defence_k', type=float, default=0.0)
    parser.add_argument('--defence_limit', action='store_true')
    parser.add_argument('--defence_visual', action='store_true')
    parser.add_argument('--defence_visual_mode', type=int, default=1)
    parser.add_argument('--defence_p', type=float, default=0.707)
